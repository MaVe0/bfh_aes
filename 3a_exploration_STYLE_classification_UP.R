...
score <- readRDS("styleScores.RDS") %>%
  rename(score = style) %>%
  mutate(score = as.factor(ifelse(score == 3, "Y", "N")))
...
testT$predProb <- predict(logit.CV, testT, type = "prob")
testT$predScore <- predict(logit.CV, testT)

colAUC(testT$predProb, testT$score, plotROC = TRUE)
