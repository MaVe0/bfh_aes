...
# get IDEA scores and flag cases that have score > 2 or score < 2
...
selFeat1 <- borutaFeatures[[1]]
selFeat3 <- borutaFeatures[[13]]
selFeatPP1 <- readRDS("sfIdea1")
selFeatPP3 <- readRDS("sfIdea3")
selFeatPPW1 <- borutaFeatures[[9]]
selFeatPPW3 <- borutaFeatures[[21]]
...
if(logReg) {
  ...
  saveRDS(saveLogReg, "IDEA_LogReg.RDS")
...
# multiclass glmnet ############################################################
  ...
  saveRDS(saveGlmNetMC, "IDEA_GlmNetMC.RDS")
...
saveRDS(saveGlmNetSW, "IDEA_GlmNetSW.RDS")
...
if(randomForest) {
  ...
  modelRandFor3
  plot(modelRandFor3)
  bla <- varImp(modelRandFor3)
  confusionMatrix(modelRandFor3)
  ...
  saveRDS(saveRandFor, "IDEA_RandFor.RDS")
...
if(randomForestMC) {
  ...
  saveRDS(saveRandForMC, "IDEA_RandForMC.RDS")
...
if(!logReg) {
  perf <- cbind(perf, readRDS("IDEA_LogReg.RDS"))
}

if(!glmNetMC) {
  perf <- cbind(perf, readRDS("IDEA_GlmNetMC.RDS"))
}

if(!glmNetSW) {
  perf <- cbind(perf, readRDS("IDEA_GlmNetSW.RDS"))
}

if(!naiveBayes) {
  perf <- cbind(perf, readRDS("IDEA_NB.RDS"))
}

if(!randomForest) {
  perf <- cbind(perf, readRDS("IDEA_RandFor.RDS"))
}

if(!randomForestMC) {
  perf <- cbind(perf, readRDS("IDEA_RandForMC.RDS"))
}
...
majVote %>%
  select(id, ideaScore = score, ideaePredScore = meanPScore) %>%
  saveRDS("IDEA_finalScore")
